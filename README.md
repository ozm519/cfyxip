...
3. **Cron** 字段填: `0,15,30,45 * * * *`

> **时区提醒**:Cloudflare 的 Cron 表达式按 **UTC** 解析。上面这行 UTC `0,15,30,45 * * * *` 表示全天每 15 分钟一次,在北京时间(UTC+8)下其实就是每小时 8:00、8:15、8:30、8:45... 这种"看着像每 15 分钟但实际偏移 8 小时"的节奏。如果你想"**北京时间整点 8、9、10...**",Cron 表达式要改成 `0 0,8,16 * * *`(对照表见"进阶配置")。
...
| 每 15 分钟(默认) | `0,15,30,45 * * * *` | 每个小时的 8:00、8:15、8:30、8:45、9:00、9:15...(全天每 15 分钟) |
| 每 30 分钟 | `0,30 * * * *` | 每个小时的 8:00、8:30、9:00、9:30... |
